...
- New files added under `backend/src/orchestration/`:
  - `estimateChunkTokens.ts` — wraps tiktoken; token-cost measurement, computed once per chunk.
  - `selectEvidence.ts` — the per-section, budget-filling selection algorithm; takes a chunk-scoring function as a parameter so scoring can be built and swapped independently.
